...
/**
  ...
  *
  *  MAP ENCODING (per the lab spec — memory-efficient uint32_t bitmaps):
  *  vertMap:   bits[31:16] = top row (16 vertical segments)
  *             bits[15:0]  = bottom row (16 vertical segments)
  *  horizMap:  bits[23:16] = top row (8 horizontal segments)
  *             bits[15:8]  = middle row (8 horizontal segments)
  *             bits[7:0]   = bottom row (8 horizontal segments)
  *
  ******************************************************************************
  */
...
PlayerMaps_t              P1_Maps  = {0, 0, 0, 0};
PlayerMaps_t              P2_Maps  = {0, 0, 0, 0};
...
char Msg_Hit[] =
    {SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,
     SPACE,SPACE,CHAR_H,CHAR_I,CHAR_T,SPACE,SPACE,
     SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE};

/* "  MISS  " — shot result */
char Msg_Miss[] =
    {SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,
     SPACE,CHAR_M,CHAR_I,CHAR_S,CHAR_S,SPACE,
     SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE};
...
char Msg_PassToP2[] =
    {SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,
     CHAR_P,CHAR_A,CHAR_S,CHAR_S,SPACE,CHAR_T,CHAR_O,SPACE,CHAR_P,CHAR_2,
     SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE};

/* "  PASS TO P1  " — hand controller to P1 */
char Msg_PassToP1[] =
    {SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,
     CHAR_P,CHAR_A,CHAR_S,CHAR_S,SPACE,CHAR_T,CHAR_O,SPACE,CHAR_P,CHAR_1,
     SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE,SPACE};
 ...
 *     Position 0 center = 256,  stays until ADC > 512+128 = 640  (> halfway to pos 1)
 *     Position 3 center = 1792, stays until ADC < 1536-128 or ADC > 2048+128
 *===========================================================================*/
uint8_t ADC_To_Grid_Hysteresis(uint16_t adc_val, uint8_t grid_size, uint8_t current_pos)
{
    ...
    if ((int16_t)adc_val < lower_thresh && current_pos > 0)
    {
        ...
        if (new_pos >= grid_size) new_pos = grid_size - 1;
        return new_pos;
    }
...
/*============================================================================
 * Cursor_Get_Bit_Index() — Convert cursor (x, y) position to a bit index
 *                          within the appropriate uint32_t bitmap.
 *
 * @param c  Pointer to cursor structure
 * @return   Bit index (0-31) into the appropriate map uint32_t
 *
 * Vertical map bit layout:
 *   bits[31:16] = top row (y=0), bit 31 = leftmost column (x=0)
 *   bits[15:0]  = bottom row (y=1), bit 15 = leftmost column (x=0)
 *
 * Horizontal map bit layout:
 *   bits[23:16] = top row (y=0), bit 23 = leftmost column (x=0)
 *   bits[15:8]  = middle row (y=1), bit 15 = leftmost column (x=0)
 *   bits[7:0]   = bottom row (y=2), bit 7 = leftmost column (x=0)
 *===========================================================================*/
uint8_t Cursor_Get_Bit_Index(Cursor_t *c)
{
    if (c->onVertical)
    {
        /* Vertical map: row 0 -> bits 31..16, row 1 -> bits 15..0 */
        if (c->y == 0)
        {
            return (uint8_t)(31 - c->x);   /* Top row: bit 31 is x=0 */
        }
        else
        {
            return (uint8_t)(15 - c->x);   /* Bottom row: bit 15 is x=0 */
        }
    }
    else
    {
        /* Horizontal map: row 0 -> bits 23..16, row 1 -> bits 15..8, row 2 -> bits 7..0 */
        if (c->y == 0)
        {
            return (uint8_t)(23 - c->x);   /* Top row */
        }
        else if (c->y == 1)
        {
            return (uint8_t)(15 - c->x);   /* Middle row */
        }
        else
        {
            return (uint8_t)(7 - c->x);    /* Bottom row */
        }
    }
}
...
void Build_Score_Message(void)
{
    ...
    for (i = 0; i < 8; i++) Msg_Score[idx++] = SPACE;
 ...
 *   - We map digit 0..7 to vert columns 0..7 (left side uses f/e, right side uses b/c)
 *     Actually, with 16 vert columns mapped to 8 digits, each digit shows 2 vert columns:
 ...
 *===========================================================================*/
void Map_To_Display(PlayerMaps_t *boats, PlayerMaps_t *shots,
                    uint8_t showBoats, Cursor_t *cursor)
{
    uint8_t digit;
    ...
    for (digit = 0; digit < 8; digit++)
    {
        ...
        {
            uint8_t bitIdx = 23 - digit;  /* horizMap bits[23:16] = top row */
            uint8_t hasBoat = BIT_READ(boats->horizMap, bitIdx);
            uint8_t hasShot = BIT_READ(shots->horizShots, bitIdx);
            ...
            else if (hasShot && !hasBoat)
            {
                ...
                if (brightMin > BRIGHT_HALF) brightMin = BRIGHT_HALF;
            }
        }

        /* Middle horizontal row — segment 'g' (bit 6) */
        {
            uint8_t bitIdx = 15 - digit;  /* horizMap bits[15:8] = middle row */
            uint8_t hasBoat = BIT_READ(boats->horizMap, bitIdx);
            uint8_t hasShot = BIT_READ(shots->horizShots, bitIdx);
        ...
        {
            uint8_t bitIdx = 7 - digit;   /* horizMap bits[7:0] = bottom row */
            uint8_t hasBoat = BIT_READ(boats->horizMap, bitIdx);
            uint8_t hasShot = BIT_READ(shots->horizShots, bitIdx);
        ...
        {
            uint8_t col = digit * 2;
            uint8_t bitIdx = 31 - col;    /* vertMap bits[31:16] = top row */
            uint8_t hasBoat = BIT_READ(boats->vertMap, bitIdx);
            uint8_t hasShot = BIT_READ(shots->vertShots, bitIdx);
        ...
        {
            uint8_t col = digit * 2;
            uint8_t bitIdx = 15 - col;    /* vertMap bits[15:0] = bottom row */
            uint8_t hasBoat = BIT_READ(boats->vertMap, bitIdx);
            uint8_t hasShot = BIT_READ(shots->vertShots, bitIdx);
        ...
        if (cursor != 0 && Blink_State)
        {
            ...
            else
            {
                ...
                {
                    cursorOnThisDigit = 1;
                    if (cursor->y == 0)      cursorSegBit = 0;  /* a (top)    */
                    else if (cursor->y == 1) cursorSegBit = 6;  /* g (middle) */
                    else                     cursorSegBit = 3;  /* d (bottom) */
                }
            }
         ...
         * Note: For common-anode displays, segments are active LOW,
         * but we store the logical pattern here. The Seven_Segment_Digit
         * function in seg7.c handles the inversion via the _7SEG lookup table.
         * Since we are writing raw bit patterns, we'll use GPIOE directly
         * in the refresh, or we store into Display_Buffer and let the ISR
         * handle the output with PWM. */
...
void Render_Display_Buffer_Digit(uint8_t digit, uint8_t pwm_tick)
{
    ...
    /* Select just this digit (active low on bits 15:8), output pattern on bits 7:0 */
 ...
 * @param orient     For double boats: 0 = extend right/down from cursor,
 *                   1 = extend in the other direction (up/left)
 * @return           1 if valid, 0 if invalid (overlap or out of bounds)
 *
 * Single boats occupy exactly one segment.
 * Double boats occupy two adjacent segments on the same row.
 *
 * Checks:
 *   1. No overlap with already-placed boats
 *   2. For doubles, the adjacent segment must be in bounds
 *===========================================================================*/
uint8_t Validate_Placement(PlayerMaps_t *maps, Cursor_t *c,
                           uint8_t isDouble, uint8_t orient)
{
    uint8_t bitIdx1, bitIdx2;
    uint32_t *map;

    /* Select the appropriate map (vertical or horizontal) */
    if (c->onVertical)
    {
        map = &(maps->vertMap);
    }
    ...
    /* Get the bit index for the first segment */
    bitIdx1 = Cursor_Get_Bit_Index(c);

    /* Check if first position is already occupied */
    if (BIT_READ(*map, bitIdx1))
    {
        return 0;  /* Overlap with existing boat */
    }

    if (isDouble)
    {
        /* Calculate second segment position for double boat */
        Cursor_t c2 = *c;  /* Copy cursor */

        if (orient == 0)
        {
            /* Extend to the right (increase x) within the same row */
            c2.x = c->x + 1;
        }
        else
        {
            /* Extend to the left (decrease x) */
            if (c->x == 0) return 0;  /* Out of bounds */
            c2.x = c->x - 1;
        }
        ...
        /* Get bit index for second segment */
        bitIdx2 = Cursor_Get_Bit_Index(&c2);

        /* Check if second position is already occupied */
        if (BIT_READ(*map, bitIdx2))
        {
            return 0;  /* Overlap */
        }
    }
 ...
 * @param orient     For doubles: 0 = extend right, 1 = extend left
 *
 * Sets the appropriate bit(s) in the boat bitmap.
 *===========================================================================*/
void Apply_Placement(PlayerMaps_t *maps, Cursor_t *c,
                     uint8_t isDouble, uint8_t orient)
{
    uint8_t bitIdx1, bitIdx2;
    uint32_t *map;
 ...
 *
 * Records the shot in the appropriate shot map and checks for a hit
 * by comparing against the boat map.
 *===========================================================================*/
uint8_t Fire_Shot(PlayerMaps_t *targetMaps, Cursor_t *cursor)
{
    ...
    if (cursor->onVertical)
    {
        boatMap = &(targetMaps->vertMap);
        shotMap = &(targetMaps->vertShots);
    }
    else
    {
        boatMap = &(targetMaps->horizMap);
        shotMap = &(targetMaps->horizShots);
    }

    bitIdx = Cursor_Get_Bit_Index(cursor);

    /* Check if this position was already shot */
    if (BIT_READ(*shotMap, bitIdx))
    {
        return 2;  /* Already fired here */
    }

    /* Record the shot */
    BIT_SET(*shotMap, bitIdx);

    /* Check if it's a hit */
    if (BIT_READ(*boatMap, bitIdx))
    {
        return 1;  /* HIT! */
    }
    else
 ...
 *
 * A boat segment is "sunk" when its position bit is set in BOTH the
 * boat map AND the shot map. We use bitwise AND to find hits, then
 * count the number of set bits.
 *===========================================================================*/
uint8_t Check_Win(PlayerMaps_t *targetMaps)
{
    uint32_t vertHits, horizHits;
    uint8_t totalHits;

    /* Find segments that are both boats AND shots (= hits) */
    vertHits  = targetMaps->vertMap  & targetMaps->vertShots;
    horizHits = targetMaps->horizMap & targetMaps->horizShots;

    /* Count total hit segments */
    totalHits = POPCOUNT32(vertHits) + POPCOUNT32(horizHits);

    return (totalHits >= TOTAL_BOAT_SEGMENTS) ? 1 : 0;
}
...
uint8_t Count_Hits(PlayerMaps_t *targetMaps)
{
    ...
    return POPCOUNT32(vertHits) + POPCOUNT32(horizHits);
}

/*============================================================================
 * Wait_For_Button() — Block until a specific button is pressed and released
 *
 * @param pin  Bitmask of the button pin to wait for (e.g., SW_FIRE_PIN)
 *
 * Polls Port C until the specified pin goes high, then waits for release
 * with debouncing.
 *===========================================================================*/
void Wait_For_Button(uint8_t pin)
{
 ...
 *===========================================================================*/
uint8_t Wait_For_Any_Button(void)
{
    uint8_t sw;
 ...
 * @param orient   0 = extend right, 1 = extend left
...
uint8_t Get_Second_Segment_Bit(Cursor_t *c, uint8_t orient, uint8_t digit)
{
    ...
    /* Calculate second segment position */
    if (orient == 0)
    {
        c2.x = c->x + 1;
    }
    ...
    /* Bounds check */
    if (c->onVertical && c2.x > 15) return 0;
    if (!c->onVertical && c2.x > 7) return 0;

    /* Check if the second segment lives on this digit */
    if (c2.onVertical)
    {
        uint8_t col2 = c2.x;
        uint8_t digitForCol = col2 / 2;
        if (digitForCol != digit) return 0;

        uint8_t isLeft = (col2 % 2 == 0) ? 1 : 0;
        if (c2.y == 0)
    ...
    {
        if (c2.x != digit) return 0;

        if (c2.y == 0)      return (1 << 0);  /* a */
        else if (c2.y == 1) return (1 << 6);  /* g */
        else                return (1 << 3);  /* d */
    }
}
 ...
 *
 * INTUITIVE TWO-STEP PLACEMENT FLOW:
 *
 *   Step 1 — AIM:
 *     A scrolling announcement tells the player which boat they are
 *     placing (e.g., "BOAT 1 SGL" or "BOAT 4 DBL"). The player moves
 *     the cursor with the potentiometers. For double boats, BOTH
 *     segments blink together as a live preview so the player can see
 *     exactly where the boat will land.
 *
 *     Controls in AIM mode:
 *       Pot X/Y       → move cursor around the grid
 *       SW2 (PC1)     → toggle between vertical/horizontal map layer
 *       SW3 (PC2)     → toggle double-boat orientation (right/left)
 *       SW1 (PC0)     → lock position and enter CONFIRM mode
 *
 *   Step 2 — CONFIRM:
 *     The cursor stops blinking and shows the boat at full brightness
 *     so the player can review the final position. The decimal points
 *     on affected digits light up as a visual "ready" indicator.
 *
 *     Controls in CONFIRM mode:
 *       SW1 (PC0)     → CONFIRM placement (boat is permanently placed)
 *       SW2 (PC1)     → CANCEL and return to AIM mode to reposition
 *
 *   After confirming, a placement beep plays and the next boat begins.
 *   If the position is invalid (overlap or out of bounds), an error
 *   buzz plays and the player stays in AIM mode.
 *===========================================================================*/
void Place_Boats(PlayerMaps_t *maps)
{
    ...
    uint8_t orient       = 0;        /* Double-boat orientation: 0=right, 1=left */
    uint8_t confirmMode  = 0;        /* 0 = AIM mode, 1 = CONFIRM mode          */
    uint8_t sw;                      /* Current switch state                     */
    uint8_t prevFire     = 0;        /* Previous fire button state (edge detect) */
    uint8_t prevMapSel   = 0;        /* Previous map-select state (edge detect)  */
    uint8_t prevOrient   = 0;        /* Previous orient state (edge detect)      */
    ...
    PlayerMaps_t emptyShots = {0, 0, 0, 0}; /* No shots during placement        */

    Boat_State = BOAT_SINGLE_1;
    Cursor.onVertical = 0;           /* Start on horizontal map                  */
    Cursor.x = 0;
    Cursor.y = 0;
    ...
    while (boatsPlaced < 5)
    {
        ...
        while (!confirmMode)
        {
            ...
            /* SW2 (PC1): Toggle vertical/horizontal map layer */
            if ((sw & SW_MAP_SEL_PIN) && !prevMapSel)
            {
                Cursor.onVertical ^= 1;
                Cursor.x = 0;
                Cursor.y = 0;
            ...
            prevMapSel = (sw & SW_MAP_SEL_PIN) ? 1 : 0;

            /* SW3 (PC2): Toggle double-boat orientation */
            if ((sw & SW_ORIENT_PIN) && !prevOrient && isDouble)
            {
                orient ^= 1;
                Play_SFX(SFX_PLACE);  /* Quick beep for feedback */
                Debounce_Delay();
            ...
            /* SW1 (PC0): Fire button — lock position for confirmation */
            if ((sw & SW_FIRE_PIN) && !prevFire)
            {
                /* Validate before entering confirm mode */
                if (Validate_Placement(maps, &Cursor, isDouble, orient))
                {
                    confirmMode = 1;  /* Enter CONFIRM mode */
                    Play_SFX(SFX_PLACE);
                }
                else
                {
            ...
            prevFire = (sw & SW_FIRE_PIN) ? 1 : 0;

            HAL_Delay(20);  /* Control loop rate */
        }

        /*==============================================================
         * CONFIRM LOOP — position locked, show solid preview
         * Press SW1 again to confirm, or SW2 to cancel & reposition
         *=============================================================*/
        while (confirmMode)
        {
            /* Show the map with the proposed boat rendered solid (not blinking) */
            Map_To_Display(maps, &emptyShots, 1, 0);  /* No blinking cursor */

            /* Overlay the proposed boat at full brightness */
            {
                /* First segment */
                uint8_t digitForSeg = 0;
                uint8_t segBit = 0;
                ...
                {
                    digitForSeg = Cursor.x;
                    if (Cursor.y == 0)      segBit = 0;
                    else if (Cursor.y == 1) segBit = 6;
                    else                    segBit = 3;
                }

                /* Light up first segment solid */
                Display_Buffer[digitForSeg] |= (1 << segBit);
                Brightness_Buffer[digitForSeg] = BRIGHT_FULL;

                /* For double boats, also light second segment solid */
                if (isDouble)
                {
                    for (digit = 0; digit < 8; digit++)
                    {
                        ...
                        if (seg2)
                        {
                            Display_Buffer[digit] |= seg2;
                            Brightness_Buffer[digit] = BRIGHT_FULL;
                        }
                    }
                }
            }
            ...
            /* Show the decimal point on affected digits as "ready" indicator */
            /* This is a subtle visual cue that we're in confirm mode */
            {
                uint8_t affectedDigit;
                if (Cursor.onVertical)
                    affectedDigit = Cursor.x / 2;
                else
                    affectedDigit = Cursor.x;
                Seven_Segment_Digit(affectedDigit, Display_Buffer[affectedDigit], 1);
            }

            /* Read switches with edge detection */
            sw = Read_Switches();

            /* SW1 (PC0): Confirm — permanently place the boat */
            if ((sw & SW_FIRE_PIN) && !prevFire)
            {
                ...
                confirmMode = 0;  /* Exit confirm loop */
                Debounce_Delay();

                /* Wait for button release before next boat */
                while (Read_Switches() & SW_FIRE_PIN) { }
                Debounce_Delay();
                break;  /* Move to next boat */
            }
            prevFire = (sw & SW_FIRE_PIN) ? 1 : 0;

            /* SW2 (PC1): Cancel — go back to AIM mode to reposition */
            if ((sw & SW_MAP_SEL_PIN) && !prevMapSel)
            {
                confirmMode = 0;  /* Return to AIM loop */

                /* Flash LEDs red briefly to indicate cancel */
                DIM_Enable = 1;
                RED_BRT   = 10;
                GREEN_BRT = 0;
                BLUE_BRT  = 0;
                ...
                Debounce_Delay();
                break;  /* Break inner loop, outer while will restart AIM */
            }
            prevMapSel = (sw & SW_MAP_SEL_PIN) ? 1 : 0;
...
void Game_Run(void)
{
     ...
     * Waits for SW_START_PIN to be pressed to proceed.
     *---------------------------------------------------------------------*/
    case STATE_WELCOME:
